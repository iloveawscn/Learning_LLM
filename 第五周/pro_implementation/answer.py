...
"""
    ...
    # Filter out-of-range indices, keep only valid ones
    valid_order = [x for x in order if 1 <= x <= len(chunks)]
    if not valid_order:
        return chunks
...
def fetch_context_unranked(question):
    query = embedding_client.embeddings.create(model=embedding_model, input=[question], timeout=30).data[0].embedding
